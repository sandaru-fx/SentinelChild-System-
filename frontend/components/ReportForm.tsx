...
export default function ReportForm() {
  ...
  if (successId) {
    return (
      <div className="max-w-2xl mx-auto mt-20 p-12 bg-white dark:bg-slate-900/60 rounded-[3rem] shadow-2xl text-center border border-slate-100 dark:border-slate-800 backdrop-blur-xl transition-colors">
        <div className="w-24 h-24 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-8 text-4xl shadow-inner">
          <i className="fas fa-check-shield"></i>
        </div>
        <h2 className="text-4xl font-black text-slate-900 dark:text-white mb-4 tracking-tighter uppercase">Inquiry Verified</h2>
        <p className="text-slate-600 dark:text-slate-400 font-bold mb-10">Your voice statement and evidence have been logged securely.</p>
  ...
  return (
    <div className="max-w-6xl mx-auto my-12 px-4 flex flex-col lg:flex-row gap-8">
      ...
      <div className="w-full lg:w-80 shrink-0 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900/40 rounded-[2.5rem] p-8 border border-slate-100 dark:border-slate-800 shadow-sm text-left relative overflow-hidden transition-colors">
          <div className="absolute top-0 right-0 p-4 text-slate-50 dark:text-slate-800/10">
            <i className="fas fa-gavel text-4xl"></i>
          </div>
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-4">Authority Notice</p>
          <p className="text-[11px] font-bold text-slate-600 dark:text-slate-400 leading-relaxed italic relative z-10">All voice statements are analyzed for forensic consistency. Providing misleading information to the Bureau is a serious criminal offense.</p>
        </div>
      </div>

      {/* Main Form Area */}
      <div className="flex-grow bg-white dark:bg-slate-900/40 rounded-[3rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col ring-1 ring-slate-900/5 dark:ring-white/5 min-h-[700px] transition-colors">
            ...
            <div className="space-y-12 animate-fade-in flex-grow flex flex-col justify-center">
              <section className="space-y-10">
                <div className="border-l-4 border-blue-600 pl-8">
                  <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Citizen Verification</h2>
                  <p className="text-sm text-slate-600 dark:text-slate-400 font-bold mt-1">Required for legal filing of an official inquiry.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      className={`w-full px-8 py-5 bg-slate-50 dark:bg-slate-950/50 border-2 ${errors.reporterName ? 'border-red-500' : 'border-slate-100 dark:border-slate-800'} rounded-[1.5rem] focus:border-blue-600 outline-none font-bold text-sm text-slate-900 dark:text-white transition-all focus:ring-4 focus:ring-blue-600/10`}
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      className={`w-full px-8 py-5 bg-slate-50 dark:bg-slate-950/50 border-2 ${errors.reporterNic ? 'border-red-500' : 'border-slate-100 dark:border-slate-800'} rounded-[1.5rem] outline-none focus:border-blue-600 font-mono text-sm text-slate-900 dark:text-white uppercase tracking-[0.2em] transition-all focus:ring-4 focus:ring-blue-600/10`}
                  ...
                  <div className="md:col-span-2 space-y-3">
                    ...
                    <input
                      ...
                      className={`w-full px-8 py-5 bg-slate-50 dark:bg-slate-950/50 border-2 ${errors.reporterPhone ? 'border-red-500' : 'border-slate-100 dark:border-slate-800'} rounded-[1.5rem] outline-none focus:border-blue-600 font-bold text-sm text-slate-900 dark:text-white transition-all focus:ring-4 focus:ring-blue-600/10`}
              ...
              <div className="pt-12 border-t border-slate-100 dark:border-slate-800 flex justify-end">
                <button type="button" onClick={nextStep} className="bg-slate-950 dark:bg-blue-600 text-white px-16 py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] hover:bg-blue-600 dark:hover:bg-blue-700 transition-all shadow-2xl hover:scale-105 active:scale-95">
            ...
            <div className="space-y-12 animate-fade-in flex-grow flex flex-col">
              <section className="space-y-10 flex-grow">
                <div className="border-l-4 border-emerald-600 pl-8">
                  <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Statement Submission</h2>
                  <p className="text-sm text-slate-600 dark:text-slate-400 font-bold mt-1">Provide incident context via Voice or Text below.</p>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {ENVIRONMENTS.map(env => (
                      <button
                        ...
                        className={`flex flex-col items-center gap-4 p-8 rounded-[2rem] border-2 transition-all duration-500 group ${formData.locationType === env.id ? 'bg-slate-950 dark:bg-blue-600 border-slate-950 dark:border-blue-600 text-white shadow-2xl scale-105' : 'bg-slate-50 dark:bg-slate-950/40 border-slate-100 dark:border-slate-800 text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'}`}
                      >
                        <i className={`fas ${env.icon} text-2xl ${formData.locationType === env.id ? 'text-blue-400 dark:text-white' : 'group-hover:text-slate-600 dark:group-hover:text-slate-300'} transition-colors`}></i>
                ...
                <div className={`relative p-10 rounded-[2.5rem] border-2 transition-all duration-700 ${isVoiceMode ? 'bg-indigo-950 border-indigo-600 shadow-2xl ring-8 ring-indigo-600/10' : 'bg-slate-50 dark:bg-slate-950/40 border-slate-100 dark:border-slate-800'}`}>
                  <div className="flex flex-col md:flex-row items-center gap-8">
                    <div className="shrink-0">
                      <button
                        type="button"
                        onClick={isVoiceMode ? stopVoiceRecording : startVoiceRecording}
                        className={`w-24 h-24 rounded-[2rem] flex items-center justify-center text-3xl transition-all duration-500 shadow-xl ${isVoiceMode ? 'bg-red-600 text-white animate-pulse' : 'bg-white dark:bg-slate-900/60 text-blue-600 dark:text-blue-400 hover:scale-110'}`}
                      >
                    ...
                    <div className="flex-grow space-y-4 text-center md:text-left">
                      <div className="flex items-center justify-center md:justify-start gap-3">
                        <h3 className={`text-lg font-black uppercase tracking-tight ${isVoiceMode ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                      ...
                      <p className={`text-xs font-bold leading-relaxed ${isVoiceMode ? 'text-indigo-200' : 'text-slate-600 dark:text-slate-400'}`}>
                    ...
                    {!isVoiceMode && (
                      <div className="text-[10px] font-black text-slate-300 dark:text-slate-700 uppercase tracking-widest border-2 border-slate-200 dark:border-slate-800 px-4 py-2 rounded-full">
                        No typing required
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-8 space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <label className={`text-[10px] font-black uppercase tracking-widest ${isVoiceMode ? 'text-indigo-400' : 'text-slate-400 dark:text-slate-600'}`}>Structured Statement Preview</label>
                    ...
                    <textarea
                      ...
                      className={`w-full px-8 py-6 rounded-[2rem] border-2 outline-none font-medium text-sm leading-relaxed transition-all ${isVoiceMode ? 'bg-white/5 border-white/10 text-white' : 'bg-white dark:bg-slate-950/60 border-slate-100 dark:border-slate-800 text-slate-800 dark:text-white focus:border-blue-600 dark:focus:border-blue-500 focus:ring-4 focus:ring-blue-600/5'}`}
                    ></textarea>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <i className="fas fa-location-dot absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-700"></i>
                    <input
                      type="text"
                      value={formData.locationText}
                      onChange={e => setFormData({ ...formData, locationText: e.target.value })}
                      placeholder="e.g. Near Galle Face Green, or 123 Main St, Kandy"
                      className="w-full pl-14 pr-8 py-5 bg-slate-50 dark:bg-slate-950/50 border-2 border-slate-100 dark:border-slate-800 rounded-[1.5rem] outline-none focus:border-emerald-600 font-bold text-sm text-slate-900 dark:text-white transition-all"
                    />
                  </div>
                </div>
              </section>

              <div className="pt-12 border-t border-slate-100 dark:border-slate-800 flex gap-6">
                <button type="button" onClick={() => setStep(1)} className="flex-1 py-6 rounded-[2rem] font-black text-xs uppercase tracking-widest text-slate-400 dark:text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-all">Previous Stage</button>
                <button type="button" onClick={nextStep} className="flex-[2] bg-slate-950 dark:bg-blue-600 text-white py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] hover:bg-blue-600 dark:hover:bg-blue-700 transition-all shadow-2xl hover:scale-105 active:scale-95">
            ...
            <div className="space-y-12 animate-fade-in flex-grow flex flex-col justify-center">
              <section className="space-y-12">
                <div className="border-l-4 border-indigo-600 pl-8">
                  <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Final Verification & Evidence</h2>
                  <p className="text-sm text-slate-600 dark:text-slate-400 font-bold mt-1">Every official inquiry MUST be accompanied by forensic evidence.</p>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-6">
                    {formData.evidence.map((src, i) => (
                      <div key={i} className="aspect-square rounded-[2rem] bg-slate-100 dark:bg-slate-900/60 border-2 border-slate-100 dark:border-slate-800 overflow-hidden relative group shadow-sm transition-colors">
                    ...
                    {formData.evidence.length < 10 && (
                      <label className={`aspect-square rounded-[2rem] border-4 border-dashed transition-all duration-500 flex flex-col items-center justify-center cursor-pointer group shadow-sm ${errors.evidence ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900/30 text-red-400' : 'bg-slate-50 dark:bg-slate-950/40 border-slate-200 dark:border-slate-800 text-slate-300 dark:text-slate-700 hover:bg-white dark:hover:bg-slate-900 hover:border-blue-600 dark:hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400'}`}>
                ...
                <div className="bg-amber-50 dark:bg-amber-900/10 p-10 rounded-[3rem] border-2 border-amber-100 dark:border-amber-900/30 space-y-6 shadow-sm transition-colors">
                  <div className="flex items-start gap-6">
                    <div className="relative">
                      <input
                        ...
                        className="w-8 h-8 rounded-xl text-blue-600 dark:text-blue-400 border-amber-200 dark:border-amber-900 bg-white dark:bg-slate-900 cursor-pointer transition-all appearance-none"
                      />
                      {formData.declaration && <i className="fas fa-check absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-600 dark:text-blue-400 pointer-events-none text-xs"></i>}
                    </div>
                    <label htmlFor="final-dec" className="text-xs font-bold text-slate-700 dark:text-slate-300 leading-relaxed cursor-pointer select-none">
                      I, <span className="text-blue-700 dark:text-blue-400 font-black">{formData.reporterName || 'undersigned'}</span>, solemnly declare that the voice statement and evidence provided are factual. I acknowledge that my identity (NIC: <span className="font-mono text-[10px] bg-white dark:bg-slate-950 px-2 py-0.5 rounded border border-amber-200 dark:border-amber-900">{formData.reporterNic || '...'}</span>) is legally bound to this inquiry.
                    </label>
                  </div>
              ...
              <div className="pt-12 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row gap-6">
                <button type="button" onClick={() => setStep(2)} className="flex-1 py-6 rounded-[2rem] font-black text-xs uppercase text-slate-400 dark:text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-all">Back to Statement</button>
                <button
                  type="submit"
                  disabled={loading}
